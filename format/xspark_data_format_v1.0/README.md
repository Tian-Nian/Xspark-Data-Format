...
> Xspark Data Format v1.0 merges all modalities into a single HDF5 file to unify data across tasks and embodiments. Images are stored with JPEG compression, and the format supports single/dual-arm setups, two-finger grippers, and dexterous hands, enabling cross-embodiment consistency.<br>
> Xspark Data Format v1.0 将所有模态数据合并到单个 HDF5 文件中，实现跨任务、跨本体的统一数据表达；图像采用 JPEG 压缩存储，并支持单双臂与二指夹爪，从而实现跨本体一致的数据组织。
